...
export default function DashboardData({ students =[],handelDelete,handelEdit }) {
  return (
    ...
    <div className=" relative w-[100%] sm:w-[90%] lg:w-3/4 bg-bg2-color rounded overflow-scroll scroll-smooth h-[100dvh]">
      {/* main content */}
      <div className="grid grid-cols-[30px_70px_1fr_70px_70px] sm:grid-cols-[repeat(2,110px)_3fr_1fr_1fr] gap-1 sm:gap-4 text-center items-center pr-5 font-bold py-2 [&_div]:p-2 bg-n-color text-gray-950 sticky top-0 ">
        <div>No</div>
        <div>Student Id</div>
      ...
      {students.map((student, index) => (
        <div
          key={student.id}
          className=" grid grid-cols-[30px_70px_1fr_70px_70px] sm:grid-cols-[repeat(2,100px)_3fr_1fr_1fr] gap-1 sm:gap-4 text-center py-2 pr-5 border-b items-center  overflow-scroll max-sm:text-sm text-gray-950"
        >
          <div>{++index}</div>
